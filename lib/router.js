	...
	Backbone.Router = {
		...
		extend: function (obj) {
			...
			app.get('*', function(req, res) {
				...
				if (!fs.existsSync((app.config.rootPath + 'public' + req.originalUrl).split('?')[0])) {

					app.log('--------------------------------------');
					app.log(req.originalUrl, 'yellow');
					app.log(JSON.stringify(req.body) || '{}', 'yellow');
					app.log('--------------------------------------\n');

					app.controllers.errors.req = req;
					app.controllers.errors.res = res;
					app.controllers.errors.render(404);
				}
